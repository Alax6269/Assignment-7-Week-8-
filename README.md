# Assignment-7-Week-8-
# Hello there, in this assignment i will be showing my program that uses both php and sql with docker to make a data table that is always updating live when user interact with the program. 

#My programcis is a personal Movie Rating Library program that allows user to list down the name of movies that they have watched at sepecific year and the user can also rate the movie. 

The user can also edit the ratimg og the movie that they previously rated or delete said movie from the library completety.

#To start using this program do as said:

#1. Download Docker Desktop.

#2. Clone this reprository to your local machine using "git clone https://github.com/Alax6269/Assignment-7-Week-8-.git" in the terminal.

#3. Open the file on Visual Studio Code app and in the app press "CTRL + ~" to open the terminal.

#4. paste these command in the terminal:
    "docker compose up"
    
    After that pres "V" on keyboard to go to Docker App   

#5. Now go to to web browser and open 2 tabs and open these pages using the URL:
     
     http://localhost:8080 

     http://localhost:8081

#6. That is the final step required, now enjoy the system.